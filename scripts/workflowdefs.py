...
stepAdapter = 'stepAdapters%(sep)s@RUNADAPTER -d @SPAIRED -pr @PREVIOUSADAPTER -o @OUTDIR -pu @PUBDIR -w @WKEY -a @ADAPTER -c @MATICCMD -s stepAdapter -j @JOB%(sep)s60'
stepQuality = 'stepQuality%(sep)s@RUNQUALITY -d @SPAIRED -pr @PREVIOUSQUALITY -o @OUTDIR -pu @PUBDIR -w @WKEY -q @QUALITY -c @MATICCMD -s stepQuality -j @JOB%(sep)s60'
...
stepAlignmentCount  = 'stepAlignmentCount%(sep)s@RUNALIGNMENTCOUNT -c @CONFIG -s @SAMTOOLS -u @USERNAME -t %(type)s -p @PUBDIR -w @WKEY -o @OUTDIR%(sep)s10'
stepSummary  = 'stepSummary%(sep)s@RUNSUMMARY -c @CONFIG -u @USERNAME -s @SAMTOOLS -p @PUBDIR -w @WKEY -o @OUTDIR%(sep)s10'
