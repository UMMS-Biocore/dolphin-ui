...
stepTophat='stepTophat2%(sep)s@PERL @RUNTOPHAT2 -a %(type)s -o @OUTDIR -g %(gtf)s -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSTOPHAT -pr @PREVIOUSPIPE -c @TOPHAT2CMD -i %(bowtie2index)s -sa @SAMTOOLS -se stepTophat2 -j @JOB%(sep)s240'
stepSTAR='stepSTAR%(sep)s@PERL @RUNSTAR -a %(type)s -o @OUTDIR -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSSTAR -pr @PREVIOUSPIPE -c @STARCMD -i %(bowtie2index)s -sa @SAMTOOLS -se stepSTAR -j @JOB%(sep)s240'
stepHisat2='stepHisat2%(sep)s@PERL @RUNHISAT2 -a %(type)s -o @OUTDIR -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSHISAT2 -pr @PREVIOUSPIPE -c @HISAT2CMD -i %(bowtie2index)s -sa @SAMTOOLS -se stepHisat2 -j @JOB%(sep)s240'
