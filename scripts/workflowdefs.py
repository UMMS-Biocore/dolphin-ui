...
stepAlignment = 'stepAlignment%(type)s%(sep)s@PERL %(script_command)s -a %(type)s -o @OUTDIR -g %(gtf)s -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSTOPHAT -pr @PREVIOUSPIPE -co %(run_command)s -i %(indexref)s -sa @SAMTOOLS -se stepAlignment%(type)s -j @JOB%(sep)s240'
#stepTophat='stepTophat2%(sep)s@PERL @RUNTOPHAT2 -a %(type)s -o @OUTDIR -g %(gtf)s -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSTOPHAT -pr @PREVIOUSPIPE -com @COMMANDTOPHAT2 -i %(bowtie2index)s -sa @SAMTOOLS -se stepTophat2 -j @JOB%(sep)s240'
#stepSTAR='stepSTAR%(sep)s@PERL @RUNSTAR -a %(type)s -o @OUTDIR -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSSTAR -pr @PREVIOUSPIPE -c @COMMANDSTAR -i %(bowtie2index)s -sa @SAMTOOLS -se stepSTAR -j @JOB%(sep)s240'
#stepHisat2='stepHisat2%(sep)s@PERL @RUNHISAT2 -a %(type)s -o @OUTDIR -d @SPAIRED -pub @PUBDIR -w @WKEY -pa @PARAMSHISAT2 -pr @PREVIOUSPIPE -c @COMMANDHISAT2 -i %(bowtie2index)s -sa @SAMTOOLS -se stepHisat2 -j @JOB%(sep)s240'
