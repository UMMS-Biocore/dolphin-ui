...
stepAlignment = 'stepAlignment%(type)s%(sep)s@PERL %(script_command)s -a %(type)s -o @OUTDIR -g %(gtf)s -d @SPAIRED -pub @PUBDIR -w @WKEY -pa %(addparameters)s -pr @PREVIOUSPIPE -co %(run_command)s -i %(indexref)s -sa @SAMTOOLS -se stepAlignment%(type)s -j @JOB%(sep)s240'
