...
stepRSEM = 'stepRSEM%(sep)s@RUNRSEM -cm @RSEMCMD -co @CONVERTRSEM -r %(rsemref)s -d @SPAIRED -g %(genome_bam)s -pa @PARAMSRSEM -pr %(previousrsem)s -o @OUTDIR  -ba %(bamsupport)s -bo @BOWTIEPATH -s stepRSEM -j @JOB%(sep)s240'
...
stepTophat='stepTophat2%(sep)s@PERL @RUNTOPHAT2 -o @OUTDIR -g %(gtf)s -d @SPAIRED -pa @PARAMSTOPHAT -pr @PREVIOUSPIPE -t @TOPHAT2CMD -b %(bowtie2index)s -sa @SAMTOOLS -se stepTophat2 -j @JOB%(sep)s240'
stepSplit='stepSplit%(sep)s@PERL @RUNSPLIT -o @OUTDIR -d @SPAIRED -p @PREVIOUSSPLIT -n @SPLIT -se stepSplit -j @JOB%(sep)s50'
stepMergeBAM='stepMergeBAM%(type)s%(sep)s@PERL @RUNMERGEBAM -o @OUTDIR -t %(type)s -d @SPAIRED -sa @SAMTOOLS -se stepMergeBAM%(type)s -j @JOB%(sep)s50'
stepMACS='stepMACS%(sep)s@RUNMACS -a @MACSCGDBMD -i "@CHIPINPUT" -t %(type)s -o @OUTDIR -s stepMacs -j @JOB%(sep)s60'
...
stepHaplotype = 'stepHaplotype%(sep)s@RUNHAPLOTYPE -o @OUTDIR -g @GENOMEFASTA -bed @BEDTOOLSSECT -ha @HAPLOTYPECMD -pi @PICARDCMD -smctfc @SMCTFC -smctfe @SMCTFE -mb @MBQS -mrp @MRPAS -mri @MRIRPS -co @COMMON -cl @CLINICAL -enh @ENHANCER -pro @PROMOTER -mo @MOTIFS -se stepHaplotype -j @JOB%(sep)s60'
